...
const gameEventDeclarations = (() => {
    const members = _.uniqBy(
        ...
        withDescription(
            `${eventName}: ${event.fields.length === 0 ? 'object' : `${_.upperFirst(_.camelCase(eventName))}Event`}`,
            event.description,
        ),
    );
...
const eventTypes = Object.values(events)
    ...
    .map(([eventName, event]) => {
        ...
        const members = event.fields
            .map(field => withDescription(`${field.name}: ${getEventType(field.type)}`, field.description))
            .join('\n');
